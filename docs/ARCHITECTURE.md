...
- Generic yt-dlp fallback: runs only after `StaticProviderResolver` returns `unsupportedProvider` for a validated public HTTPS URL. It exposes bounded safe quality labels/media kinds, retains the original source URL in every projected quality, and re-extracts into a private single-output workspace for transfer. Signed media URLs, extractor traces, headers, and filesystem paths never enter durable jobs or Cloud preview results. The generic path never receives the PornHub Keychain session or cookie file.
...
- Generic yt-dlp materialization uses the same bounded process runner, progress protocol, private single-output workspace, output validation, and cleanup rules. Its selector is produced by the trusted metadata projection rather than accepted as an arbitrary process argument.
...
- Feed extraction is separate from queueing. The browser creates an account/device-scoped resolve command for the canonical source page, the paired agent performs fresh provider resolution, and the response exposes only bounded playback candidates and safe required headers. These temporary URLs are intended for direct network playback and remain browser-session data.
- Watchlist source metadata and watched state are account-backed in Neon. Resolution always starts from the retained public source page so expired provider URLs can be refreshed. The browser holds resolved playback candidates by Watchlist item ID and presents them in a modal; transient media URLs are not persisted in the Watchlist table.
- Raw SQL repository returns are normalized from database snake case into the application contract at the repository boundary. Command creation and Watchlist API responses must not cast raw Drizzle results directly to camel-case domain types.
...
Feed command creation first looks for the exact account/device/kind/site/normalized-query/page result and returns it as fresh (up to five minutes) or stale (up to 60 minutes) while committing a live refresh. Identical pending/running commands are coalesced. The dashboard can render cached metadata immediately, but it cannot queue from that view until the live result succeeds.
