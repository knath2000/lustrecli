...
window.addEventListener("message", (event) => {
  ...
  if (message.type === "capture_allpornstream_feed") {
    ...
    api.runtime.sendMessage(message).then((response) => {
      ...
      window.postMessage({ source: "lustre-watch-extension", requestID: message.requestID, type: "capture_failed", message: "Extension feed capture failed." }, origin);
    });
    return;
  }
  let url;
  ...
  api.runtime.sendMessage(message).then((response) => {
    ...
    window.postMessage({ source: "lustre-watch-extension", requestID: message.requestID, type: "capture_failed", message: "Extension capture failed." }, origin);
  });
});
