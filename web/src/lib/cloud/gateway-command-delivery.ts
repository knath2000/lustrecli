...
export function gatewayCommandHandler(select: SelectGatewayCommand) {
  return async (request: Request) => {
    try {
      ...
      if (command?.kind === "jobs_reorder" && (
        Object.keys(command.payload).sort().join(",") !== "deliveryProtocol,jobIDs" ||
        command.payload.deliveryProtocol !== "gateway-v1" ||
        !Array.isArray(command.payload.jobIDs) ||
        command.payload.jobIDs.length < 1 ||
        command.payload.jobIDs.length > 50 ||
        !command.payload.jobIDs.every((id) => typeof id === "string" && UUID_PATTERN.test(id)) ||
        new Set(command.payload.jobIDs).size !== command.payload.jobIDs.length
      )) throw new DeviceContractError("invalid_request", "Invalid queued download order.");
