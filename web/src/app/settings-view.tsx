  ...
  return <div className="settings-page">
    ...
    <div className="settings-layout">
      ...
      <div className="settings-sections">
        ...
        <section className="settings-section glass-panel" id="pornhub"><header><div><p className="eyebrow">Provider session</p><h3>PornHub</h3></div><span className={`settings-status ${pornHubAuth?.state === "signedIn" ? "connected" : ""}`}><i />{pornHubAuth?.state === "signedIn" ? "Signed in" : pornHubAuth?.state === "signingIn" ? "Signing in" : pornHubAuth?.state === "expired" ? "Expired" : "Signed out"}</span></header><div className="settings-row"><div><strong>Visible sign-in only</strong><p>Sign-in opens a separate macOS PornHub window. Your credentials go directly to PornHub; Lustre never reads or stores them.</p></div><span>Keychain session</span></div><div className="settings-row"><div><strong>Authenticated feeds</strong><p>Subscriptions, liked videos, and favorites use the local agent’s private session only.</p></div><span>{pornHubAuth?.lastValidatedAt ? "Validated" : "Not validated"}</span></div><footer>{pornHubAuth?.state === "signedIn" ? <button className="settings-danger" disabled={authWorking} onClick={() => { setAuthWorking(true); void onPornHubSignOut().finally(() => setAuthWorking(false)); }}>{authWorking ? "Signing out…" : "Sign out"}</button> : <button className="secondary-button" disabled={!connected || authWorking || pornHubAuth?.state === "signingIn"} onClick={() => { setAuthWorking(true); void onPornHubSignIn().finally(() => setAuthWorking(false)); }}>{authWorking || pornHubAuth?.state === "signingIn" ? "Waiting for sign-in…" : "Sign in with PornHub"}</button>}</footer></section>
