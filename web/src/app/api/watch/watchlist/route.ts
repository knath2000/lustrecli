...
export async function GET() {
  try {
    ...
    return Response.json(await watchDB().unsafe(`SELECT ${select} FROM lustre_watchlist_items WHERE account_id = $1 AND deleted_at IS NULL ORDER BY updated_at DESC`, [account]));
...
export async function POST(request: Request) {
  try {
    ...
    const rows = await watchDB().unsafe(`INSERT INTO lustre_watchlist_items (account_id, source_page_url, title, provider, thumbnail_url) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (account_id, source_page_url) DO UPDATE SET title=EXCLUDED.title, provider=EXCLUDED.provider, thumbnail_url=COALESCE(EXCLUDED.thumbnail_url,lustre_watchlist_items.thumbnail_url), deleted_at=NULL, updated_at=now() RETURNING ${select}`, [account, url.href, body.title.slice(0, 1000), body.provider.slice(0, 80), thumbnail?.href ?? null]);
    await watchDB()`INSERT INTO lustre_collection_changes (account_id, entity_type, entity_id, operation, payload) VALUES (${account}, 'watchlist', ${rows[0].id}, 'upsert', ${watchDB().json(rows[0])})`;
