...
import { cachedFeedResult, feedCommand, feedQueueCommand, feedResolveCommand, homeWorkspaceCommand, jobActionCommand, jobsReorderCommand, libraryCommand, pornHubAuthCommand, queueURLCommand, watchlistQueueCommand, watchlistResolveCommand } from "@/lib/cloud/device-repository";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const reorderJobIDs = body.kind === "jobs_reorder" && Array.isArray(body.jobIDs) && body.jobIDs.length > 0 && body.jobIDs.length <= 50 && body.jobIDs.every((id) => typeof id === "string" && uuidPattern.test(id)) && new Set(body.jobIDs).size === body.jobIDs.length
      ? body.jobIDs as string[]
      : null;
    if (body.kind === "jobs_reorder" && !reorderJobIDs) throw new DeviceContractError("invalid_request", "A valid queued download order is required.");
