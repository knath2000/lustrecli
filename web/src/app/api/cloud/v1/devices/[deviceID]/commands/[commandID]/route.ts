...
export async function GET(_request: Request, context: RouteContext) { try { const account = await requireCurrentAccount(); const { deviceID, commandID } = await context.params; const command = await commandForOwnedDevice(account.id, deviceID, commandID); return Response.json({ command: { id: command.id, status: command.status, result: command.result } }, { headers: { "Cache-Control": "no-store" } }); } catch (error) { return jsonError(error); } }
