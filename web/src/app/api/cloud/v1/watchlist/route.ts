...
export async function GET() {
  try {
    ...
    const items = await db.select().from(lustreWatchlistItems).where(eq(lustreWatchlistItems.accountID, account.id)).orderBy(desc(lustreWatchlistItems.updatedAt)).limit(500);
...
export async function POST(request: Request) {
  try {
    ...
    const [item] = await db.insert(lustreWatchlistItems).values({
      ...
      set: { title: body.title.trim(), provider: body.provider.trim(), thumbnailURL, updatedAt: new Date() },
    }).returning();
...
export async function DELETE(request: Request) {
  try {
    ...
    await db.delete(lustreWatchlistItems).where(and(eq(lustreWatchlistItems.id, body.id), eq(lustreWatchlistItems.accountID, account.id)));
