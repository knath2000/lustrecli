...
async function agentRequest<T>(deviceID: string, path: string, options: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(target, { method: options.method === "GET" ? "GET" : "POST", headers: { "Content-Type": "application/json" }, body: body ? JSON.stringify(body) : undefined, cache: "no-store" });
