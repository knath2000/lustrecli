...
async function agentRequest<T>(deviceID: string, path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (path.startsWith("/v1/feed/")) { const id = payload.command.id; for (let attempt = 0; attempt < 15; attempt += 1) { await new Promise((resolve) => window.setTimeout(resolve, 2_000)); const result = await fetch(`${base}/commands/${id}`, { cache: "no-store" }).then((value) => value.json()); if (result.command.status === "completed") return result.command.result.sites ?? result.command.result.page; if (result.command.status === "failed") throw new Error("The paired Mac could not load this feed."); } throw new Error("The paired Mac did not answer the feed request in time."); }
