...
const navigation = [["Devices", "devices"], ["Downloads", "downloads"], ["Destinations", "destinations"], ["Activity", "activity"], ["Settings", "settings"]] as const;
...
function availableActions(status: JobStatus): JobAction[] { if (status === "queued" || status === "running") return ["pause", "cancel"]; if (status === "paused") return ["resume", "cancel"]; if (["failed", "cancelled", "verificationRequired"].includes(status)) return ["retry", ...(status === "verificationRequired" ? ["cancel" as const] : [])]; return []; }
...
export default function Home() {
  ...
  const refresh = useCallback(async (activeToken = token, force = false) => {
    ...
    const promise = Promise.all([agentRequest<DownloadJob[]>(activeToken, "/v1/jobs"), agentRequest<Destination[]>(activeToken, "/v1/destinations")]).then(([nextJobs, nextDestinations]) => {
      ...
      setJobs(nextJobs); setDestinations(nextDestinations); setConnected(true); setError(null);
    });
